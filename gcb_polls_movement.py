...
def _normalize_gcb_polls(df: pd.DataFrame) -> pd.DataFrame:
    def _convert_date_str_to_dttm(date_str) -> datetime.date:
...
def _split_on_date_and_merge_again(df: pd.DataFrame, first_date: tuple, split_date: tuple) -> pd.DataFrame:
    data = df.copy()
...
def _split_on_date_and_merge_again_and_save(df: pd.DataFrame, label: str, *args) -> None:
    _split_on_date_and_merge_again(df, *args).to_csv(f'{FOLDER}{label}.csv', index=False)
    return


def create_gcb_polls_movement_trackers(df: pd.DataFrame) -> None:
    df = _normalize_gcb_polls(df)

    _split_on_date_and_merge_again_and_save(
        df, '1-Dobbs (YTD split at 6.24)', (2022, 1, 1), (2022, 6, 24))
    _split_on_date_and_merge_again_and_save(
        df, '2-Student Loan Forgiveness (6.24-8.24 vs 8.24-Today)', (2022, 6, 24), (2022, 8, 24))
    return


def create_gcb_polls_trimmed() -> None:
    df = _normalize_gcb_polls(_read_gcb_polls(['cycle']))[[
    ...
    df['endYear'] = df.endDate.apply(lambda x: x.year)
